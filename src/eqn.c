...
Consists of:
    record type (16)
    [options] model is RGB unless mtefCOLOR_CMYK bit is set; 
            type is process unless mtefCOLOR_SPOT bit is set; 
            color is unnamed unless mtefCOLOR_NAME bit is set
    [color values] if RGB, 3 values (red, green, blue); 
            if CMYK, 4 values (cyan, magenta, yellow, black); 
            see below for details
    [name] null-terminated color name; 
        appears only if mtefCOLOR_NAME option is set
    This record defines a color (see Definition records). 
    Each color value is written as a 16-bit integer that ranges between 0 and 1000 where 0 is the absence of the color and 1000 is a fully saturated color. 
    So, an RGB color definition for black has all three components at 0.
*/
        case COLOR_DEF:
            (*src_index)++;     /* skip tag */
            (*src_index)++;     /* options */
